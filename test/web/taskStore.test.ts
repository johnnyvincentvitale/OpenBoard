...
function makeFakeClient(overrides: Partial<TaskClientLike> = {}): TaskClientLike {
  return {
    ...
    addLink: vi.fn(async () => makeTask({ id: "new", parentIds: ["parent-1"] })),
    removeLink: vi.fn(async () => makeTask({ id: "new", parentIds: [] })),
...
describe("createTaskStore — worktree actions + settings", () => {
  ...
  it("addLink and removeLink update parentIds in state", async () => {
    const { connect, handle } = makeFakeConnect();
    const linked = makeTask({ id: "a", parentIds: ["parent-1"] });
    const unlinked = makeTask({ id: "a", parentIds: [] });
    const client = makeFakeClient({
      addLink: vi.fn(async () => linked),
      removeLink: vi.fn(async () => unlinked),
    });
    const store = createTaskStore({ client, connect, healthPollMs: 1_000_000 });
    store.init();
    await flush();
    ...
    await store.addLink("a", "parent-1");
    expect(client.addLink).toHaveBeenCalledWith("a", "parent-1");
    expect(store.getSnapshot().tasks[0]?.parentIds).toEqual(["parent-1"]);

    await store.removeLink("a", "parent-1");
    expect(client.removeLink).toHaveBeenCalledWith("a", "parent-1");
    expect(store.getSnapshot().tasks[0]?.parentIds).toEqual([]);
  });

  it("keeps parentIds in sync when SSE snapshots replace tasks", async () => {
    const { connect, handle } = makeFakeConnect();
    const client = makeFakeClient();
    const store = createTaskStore({ client, connect, healthPollMs: 1_000_000 });
    store.init();
    ...
    handle.handlers?.onFrame({ kind: "snapshot", seq: 1, tasks: [makeTask({ id: "a", parentIds: ["p1"] })] });
    expect(store.getSnapshot().tasks[0]?.parentIds).toEqual(["p1"]);

    handle.handlers?.onFrame({ kind: "snapshot", seq: 2, tasks: [makeTask({ id: "a", parentIds: ["p2"] })] });
    expect(store.getSnapshot().tasks[0]?.parentIds).toEqual(["p2"]);
  });
});
