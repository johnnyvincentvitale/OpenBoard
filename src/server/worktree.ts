 ...
 *   - integrate: merge the worktree branch *into* the base branch, then remove the
...
export interface WorktreeManager {
  ...
  /** Merge `branch` into `targetBranch` in the main repo, then remove `worktreePath` (keep branch). */
  integrate(
    repoDir: string,
    branch: string,
    targetBranch: string,
...
export class GitWorktreeManager implements WorktreeManager {
  ...
  async initRepo(dir: string, message = "opencode-board: initial commit"): Promise<void> {
    ...
    const commit = await git(dir, [
      "-c",
      "user.name=opencode-board",
      "-c",
      "user.email=opencode-board@localhost",
      "commit",
      "--no-gpg-sign",
      "-m",
      message,
