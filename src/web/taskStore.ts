...
export interface TaskClientLike {
  ...
  addLink: typeof taskClient.addLink;
  removeLink: typeof taskClient.removeLink;
...
export function createTaskStore(deps: TaskStoreDeps = {}): TaskStore {
  ...
  async function addLink(taskId: string, parentId: string): Promise<void> {
    const existing = tasksById.get(taskId);
    if (existing) {
      tasksById.set(taskId, {
        ...existing,
        parentIds: [...new Set([...(existing.parentIds ?? []), parentId])],
      });
      emit();
    }

    try {
      const updated = await client.addLink(taskId, parentId);
  ...
  async function removeLink(taskId: string, parentId: string): Promise<void> {
    const existing = tasksById.get(taskId);
    if (existing) {
      tasksById.set(taskId, {
        ...existing,
        parentIds: (existing.parentIds ?? []).filter((id) => id !== parentId),
      });
      emit();
    }

    try {
      const updated = await client.removeLink(taskId, parentId);
      if (updated && typeof updated === "object" && "id" in updated) {
        tasksById.set(updated.id, updated);
        emit();
      }
