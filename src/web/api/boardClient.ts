...
export async function getBoard(): Promise<Card[]> {
  const res = await fetch(buildPath.board(), { headers: boardAuthHeaders() });
...
export async function interrupt(id: string): Promise<void> {
  const res = await fetch(buildPath.cardInterrupt(id), {
    method: "POST",
...
export async function diff(id: string): Promise<unknown> {
  const res = await fetch(buildPath.cardDiff(id), { headers: boardAuthHeaders() });
  const body = await assertOk(res);
  return body;
}
